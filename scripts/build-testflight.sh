...
#
#   scripts/build-testflight.sh            # archive + export dist/ClaudeUsageTrendTracker-<ver>-<build>.pkg
...
#   - Provisioning profile "Claude Usage Trend Tracker Mac App Store" (minted via ASC API, see scripts/asc.py)
...
ARCHIVE="build/ClaudeUsageTrendTracker-AppStore.xcarchive"; OUT="build/testflight-export"
echo "==> xcodegen generate"; xcodegen generate >/dev/null
echo "==> archive Release-AppStore $VER ($BUILD)"
xcodebuild -project ClaudeUsageTrendTracker.xcodeproj -scheme ClaudeUsageTrendTracker -configuration Release-AppStore \
...
if [[ $UPLOAD == 1 ]]; then
  ...
  mkdir -p dist; cp "$OUT"/*.pkg "dist/ClaudeUsageTrendTracker-$VER-$BUILD.pkg"; ls -la dist/
fi
